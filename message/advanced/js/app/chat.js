...
(function($applozic, w, d) {
	...
	$applozic.fn.applozic = function(appOptions, params) {
		...
		if ($applozic.type(appOptions) === "object") {
			appOptions = $applozic
					.extend(true, {}, default_options, appOptions);
		}
		...
		if (typeof ($mck_sidebox.data("applozic_instance")) !== "undefined") {
			...
			if ($applozic.type(appOptions) === "string") {
				...
				case "loadBroadcastTab":
					params.groupName = (params.groupName) ? params.groupName
							: 'Broadcast';
					params.type = 5;
			...
			if (appOptions.userId && appOptions.appId
				...
				if (typeof ($mck_sidebox.data("applozic_instance")) !== "undefined") {
					...
					if (typeof appOptions.omckm === "function") {
						...
						$applozic.fn.mckModal = $applozic.fn.mckModal
								.noConflict();
	...
	function Applozic(appOptions) {
		...
		var MCK_USER_ID = (IS_MCK_VISITOR) ? "guest" : $applozic
				.trim(appOptions.userId);
		var MCK_NOTIFICATION_ICON_LINK = appOptions.notificationIconLink;
		var MCK_GOOGLE_API_KEY = (IS_MCK_LOCSHARE) ? appOptions.googleApiKey
				: "NO_ACCESS";
		var IS_MCK_TOPIC_BOX = (typeof appOptions.topicBox === "boolean") ? (appOptions.topicBox)
				: false;
		var IS_MCK_OL_STATUS = (typeof appOptions.olStatus === "boolean") ? (appOptions.olStatus)
				: false;
		var MCK_CHECK_USER_BUSY_STATUS = (typeof appOptions.checkUserBusyWithStatus === "boolean") ? (appOptions.checkUserBusyWithStatus)
				: false;
		var IS_MCK_TOPIC_HEADER = (typeof appOptions.topicHeader === "boolean") ? (appOptions.topicHeader)
				: false;
		var MCK_SUPPORT_ID_DATA_ATTR = (appOptions.supportId) ? ('data-mck-id="'
				+ appOptions.supportId + '"')
				: '';
		var IS_MCK_OWN_CONTACTS = (typeof appOptions.loadOwnContacts === "boolean") ? (appOptions.loadOwnContacts)
				: false;
		var IS_MCK_NOTIFICATION = (typeof appOptions.desktopNotification === "boolean") ? appOptions.desktopNotification
				: false;
		var IS_NOTIFICATION_ENABLED = (typeof appOptions.notification === "boolean") ? appOptions.notification
				: true;
		var IS_LAUNCH_ON_UNREAD_MESSAGE_ENABLED = (typeof appOptions.launchOnUnreadMessage === "boolean") ? appOptions.launchOnUnreadMessage
				: false;
		var CONVERSATION_STATUS_MAP = [ "DEFAULT", "NEW", "OPEN" ];
		var GROUP_TYPE_MAP = [ 1, 2, 5 ];
		var BLOCK_STATUS_MAP = [ "BLOCKED_TO", "BLOCKED_BY", "UNBLOCKED_TO",
				"UNBLOCKED_BY" ];
		...
		_this.reInit = function(optns) {
			...
			MCK_USER_ID = (IS_MCK_VISITOR) ? "guest" : $applozic
					.trim(optns.userId);
			MCK_AUTHENTICATION_TYPE_ID = optns.authenticationTypeId;
			MCK_GOOGLE_API_KEY = (IS_MCK_LOCSHARE) ? optns.googleApiKey
					: "NO_ACCESS";
			IS_MCK_OL_STATUS = (typeof optns.olStatus === "boolean") ? (optns.olStatus)
					: false;
			IS_MCK_TOPIC_BOX = (typeof optns.topicBox === "boolean") ? (optns.topicBox)
					: false;
			IS_MCK_TOPIC_HEADER = (typeof optns.topicHeader === "boolean") ? (optns.topicHeader)
					: false;
			IS_NOTIFICATION_ENABLED = (typeof optns.notification === "boolean") ? optns.notification
					: true;
			MCK_SUPPORT_ID_DATA_ATTR = (optns.supportId) ? ('data-mck-id="'
					+ optns.supportId + '"') : '';
			IS_MCK_NOTIFICATION = (typeof optns.desktopNotification === "boolean") ? optns.desktopNotification
					: false;
			IS_MCK_OWN_CONTACTS = (typeof optns.loadOwnContacts === "boolean") ? (optns.loadOwnContacts)
					: false;
			MCK_CHECK_USER_BUSY_STATUS = (typeof optns.checkUserBusyWithStatus === "boolean") ? (optns.checkUserBusyWithStatus)
					: false;
			IS_LAUNCH_ON_UNREAD_MESSAGE_ENABLED = (typeof optns.launchOnUnreadMessage === "boolean") ? optns.launchOnUnreadMessage
					: false;
			if (optns.userId && optns.appId
				...
				w
						.console("Oops! looks like incorrect application id or user Id.");
				return;
			}
		};
		...
		_this.addWelcomeMessage = function(params) {
			if (typeof params === "object") {
				...
				if (typeof params.messageContent === 'undefined'
		...
		_this.initGroupTab = function(params) {
			if (typeof params === "object") {
				...
				if (users.length > MCK_GROUPMAXSIZE) {
					return "Users limit exceeds " + MCK_GROUPMAXSIZE
		...
		function MckUtils() {
			...
			_this.getLauncherHtml = function() {
				return '<div id="mck-sidebox-launcher" class="mck-sidebox-launcher">'
						...
						+ (MCK_MODE === 'support' ? MCK_SUPPORT_ID_DATA_ATTR
								: '')
						+ '><span class="mck-icon-chat"></span></a></div>'
			...
			_this.initializeApp = function(optns, isReInit) {
				...
				if (AUTHENTICATION_TYPE_ID_MAP
						.indexOf(MCK_AUTHENTICATION_TYPE_ID) === -1) {
						...
						.ajax({
							...
							success : function(result) {
								...
								if (typeof result === 'object'
									...
									$applozic(".applozic-launcher").each(
											function() {
												if (!$applozic(this).hasClass(
														"mck-msg-preview")) {
													$applozic(this).show();
												}
											});
									...
									if (result.betaPackage) {
										var poweredByUrl = "https://www.applozic.com/?utm_source="
												+ w.location.href
												+ "&utm_medium=webplugin&utm_campaign=poweredby";
										$applozic(".mck-running-on a").attr(
												'href', poweredByUrl);
										$applozic(".mck-running-on")
												.removeClass("n-vis").addClass(
														'vis');
									}
									if (!IS_MCK_VISITOR
											&& MCK_USER_ID !== "guest"
											&& MCK_USER_ID !== "0"
											&& MCK_USER_ID !== "C0") {
										(isReInit) ? mckInitializeChannel
												.reconnect()
												: mckInitializeChannel.init();
										// mckGroupService.loadGroups();
									}
									mckStorage.clearMckMessageArray();
									var mckContactNameArray = mckStorage
											.getMckContactNameArray();
									...
									w.console
											.log("Unable to initiate app. Please reload page.");
									if (typeof MCK_ON_PLUGIN_INIT === "function") {
										MCK_ON_PLUGIN_INIT("error");
									}
								}
							...
							error : function() {
								w.console
										.log('Unable to initialize app. Please reload page.');
								if (typeof MCK_ON_PLUGIN_INIT === "function") {
									MCK_ON_PLUGIN_INIT("error");
								}
							}
						...
						.on(
								...
								function(e) {
									...
									if (contentType.indexOf("video") !== -1) {
										var videoTag = $this.find(
												'.mck-video-box').html(), video;
										$this
												.fancybox({
													content : videoTag,
			...
			_this.mouseX = function(evt) {
				if (evt.pageX) {
					...
					return evt.clientX
							+ (d.documentElement.scrollLeft ? d.documentElement.scrollLeft
									: d.body.scrollLeft);
				} else {
					return null;
				}
			...
			_this.mouseY = function(evt) {
				if (evt.pageY) {
					...
					return evt.clientY
							+ (d.documentElement.scrollTop ? d.documentElement.scrollTop
									: d.body.scrollTop);
				} else {
					return null;
				}
		...
		function MckMessageService() {
			...
			$applozic(d).on(
					...
					".mck-message-delete",
					function() {
						_this.deleteMessage($applozic(this).parents('.mck-m-b')
								.data("msgkey"));
					});
			$applozic(".mck-minimize-icon").click(function() {
				...
				if ($mck_sidebox_content.hasClass("minimized")) {
					...
					_this.initSearch = function() {
						$mck_contacts_content.removeClass('vis').addClass(
								'n-vis');
						$mck_sidebox_content.removeClass('vis').addClass(
								'n-vis');
						$mck_group_info_tab.removeClass('vis')
								.addClass('n-vis');
						$mck_sidebox_search.removeClass('n-vis')
								.addClass('vis');
						$mck_search_inner
								.html('<ul id="mck-search-list" class="mck-search-list mck-contact-list mck-nav mck-nav-tabs mck-nav-stacked"></ul>');
						if (MCK_CONTACT_ARRAY.length !== 0) {
							...
							$mck_search_inner
									.html('<div class="mck-no-data-text mck-text-muted">No contacts yet!</div>');
					...
					_this.init = function() {
						...
						$mck_text_box
								.keydown(function(e) {
									...
									if (e.keyCode === 13
										...
										e.preventDefault();
										if (w.getSelection) {
											var selection = w.getSelection(), range = selection
													.getRangeAt(0), br = d
													.createElement("br"), textNode = d
													.createTextNode("\u00a0"); // Passing
											// " "
											// directly
											// will
											// not
										...
										($mck_msg_sbmt.is(':disabled') && $mck_file_box
												.hasClass('vis')) ? alert('Please wait file is uploading.')
												: $mck_msg_form.submit();
									} else if (MCK_TYPING_STATUS === 0) {
										mckInitializeChannel.sendTypingStatus(
												1, mckMessageLayout
														.getMckMessageInner()
														.data('mck-id'));
									}
						...
						$applozic(d)
								.on(
										"click",
										".applozic-tm-launcher",
										function(e) {
											e.preventDefault();
											var tabId = $applozic(this).data(
													"mck-id");
											tabId = (typeof tabId !== "undefined" && tabId !== "") ? tabId
													.toString()
													: "";
											var userName = $applozic(this)
													.data("mck-name");
											userName = (typeof userName !== "undefined" && userName !== "") ? userName
													.toString()
													: "";
											var supportId = $applozic(this)
													.data("mck-supportid");
											supportId = (typeof supportId !== "undefined" && supportId !== "") ? supportId
													.toString()
													: "";
											var topicId = $applozic(this).data(
													"mck-topicid");
											topicId = (typeof topicId !== "undefined" && topicId !== "") ? topicId
													.toString()
													: "";
											var msgText = $applozic(this).data(
													"mck-msg");
											msgText = (typeof msgText !== "undefined" && msgText !== "") ? msgText
													.toString()
													: "";
											...
											if (supportId) {
												...
												mckMessageLayout
														.loadTab(
																params,
																mckMessageService.dispatchMessage);
											}
										});
						$applozic(d)
								.on(
										...
										function(e) {
											...
											topicId = (typeof topicId !== "undefined" && topicId !== "") ? topicId
													.toString()
													: "";
											var topicStatus = $applozic(this)
													.data("mck-topic-status");
											if (topicStatus) {
												topicStatus = (CONVERSATION_STATUS_MAP
														.indexOf(topicStatus) === -1) ? CONVERSATION_STATUS_MAP[0]
														: topicStatus
																.toString();
											} else {
										...
										function(e) {
											...
											if (typeof (MCK_GETCONVERSATIONDETAIL) === "function") {
												...
												if (typeof conversationDetail === 'object') {
													if (conversationDetail.topicDetail
													...
													if (conversationDetail.fallBackTemplatesList
						...
						$applozic(d)
								.on(
										...
										".left .person,."
												+ MCK_LAUNCHER
												+ ",.mck-conversation-tab-link, .mck-contact-list ."
												+ MCK_LAUNCHER,
										function(e) {
											e.preventDefault();
											...
											conversationId = (typeof conversationId !== "undefined" && conversationId !== "") ? conversationId
													.toString()
													: "";
						...
						$applozic(d)
								.on(
										...
										function(e) {
											...
											$mck_msg_inner = mckMessageLayout
													.getMckMessageInner();
											var conversationId = $mck_msg_inner
													.data('mck-conversationid');
											...
											mckInitializeChannel
													.unsubscibeToTypingChannel();
										});
						$applozic(d).on("click", ".mck-price-submit",
								function(e) {
									e.preventDefault();
						...
						$mck_contacts_inner
								.bind(
										'scroll',
										function() {
											if ($mck_contacts_inner.scrollTop()
													+ $mck_contacts_inner
															.innerHeight() >= $mck_contacts_inner[0].scrollHeight) {
												var startTime = $mck_contacts_inner
														.data('datetime');
												if (startTime > 0
														&& !CONTACT_SYNCING) {
													mckMessageService
						...
						$mck_block_button
								.on(
										...
										function(e) {
											...
											$mck_msg_inner = mckMessageLayout
													.getMckMessageInner();
											var tabId = $mck_msg_inner
													.data('mck-id');
											var isGroup = $mck_msg_inner
													.data('isgroup');
											var isBlock = !$mck_msg_inner
													.data('blocked');
											if (isGroup) {
												$li_mck_block_user.removeClass(
														'vis')
														.addClass('n-vis');
												return;
											}
											var blockText = (isBlock) ? 'Are you sure want to block this user!'
													: 'Are you sure want to unblock this user!';
											if (confirm(blockText)) {
												mckContactService.blockUser(
														tabId, isBlock);
											}
										});
						$mck_leave_group_btn
						...
						$applozic(d)
								.on(
										...
										function(e) {
											e.preventDefault();
											var userId = $applozic(this).data(
													'mck-id');
											if (typeof userId !== 'undefined') {
												mckGroupLayout
														.addGroupMemberFromSearch(userId);
											}
											$mck_goup_search_box
													.mckModal('hide');
										});
										...
										function(e) {
											...
											var userId = $applozic(this)
													.parents(
															'.mck-li-group-member')
													.data('mck-id');
											var groupId = $mck_group_info_tab
													.data('mck-id');
						...
						$mck_group_info_btn.on('click', function(e) {
							...
							if (!isGroup) {
								$mck_group_menu_options.removeClass('vis')
										.addClass('n-vis');
								return;
							}
							var params = {
						...
						$mck_group_back_link.on('click', function(e) {
							...
							var conversationId = $mck_group_info_tab
									.data('mck-conversation-id');
							if (groupId) {
								var params = {
									'tabId' : groupId,
									'isGroup' : true
						...
						$mck_group_search
								.keypress(function(e) {
									if (e.which === 13) {
										...
										if (userId !== "") {
											var contact = mckMessageLayout
													.getContact('' + userId);
											(typeof contact === 'object') ? mckGroupLayout
													.addGroupMemberFromSearch(contact.contactId)
						...
						$applozic(d)
								.on(
										"click",
										".mck-group-tab-search",
										function(e) {
											e.preventDefault();
											var userId = $mck_group_search
													.val();
											if (userId !== "") {
												var contact = mckMessageLayout
														.getContact('' + userId);
						...
						$applozic(d)
								.on(
										...
										".mck-accept",
										function(e) {
											var conversationId = $applozic(this)
													.data('mck-conversationid');
											var priceText = $applozic(this)
													.data('mck-topic-price');
											if (typeof (MCK_PRICE_DETAIL) === 'function'
													&& priceText
													&& conversationId) {
												var conversationPxy = MCK_CONVERSATION_MAP[conversationId];
												var groupId = $mck_msg_to.val();
												var supplierId = mckGroupLayout
												...
												if (typeof conversationPxy === 'object') {
													...
													mckMessageService
															.sendConversationCloseUpdate(conversationId);
												} else {
													mckMessageService
															.getTopicId({
																'conversationId' : conversationId,
						...
						$mck_msg_form
								.submit(function(e) {
									...
									var message = $applozic.trim(mckUtils
											.textVal($mck_text_box[0]));
									if ($mck_file_box.hasClass('n-vis')
											&& FILE_META.length > 0) {
										FILE_META = [];
									}
									if (message.length === 0
									...
									if (typeof (MCK_MSG_VALIDATION) === 'function'
									...
									if (conversationId) {
										...
										if (typeof topicDetail === "object") {
											conversationPxy.topicDetail = w.JSON
													.stringify(topicDetail);
										}
									...
									$mck_msg_error.removeClass('vis').addClass(
											'n-vis');
									$mck_msg_error.html("");
									$mck_response_text.html("");
									$mck_msg_response.removeClass('vis')
											.addClass('n-vis');
									_this.sendMessage(messagePxy);
									return false;
								});
						$mck_form_field.on('click',
								function() {
									$applozic(this).val("");
									$mck_msg_error.removeClass('vis').addClass(
											'n-vis');
			...
			_this.sendMessage = function(messagePxy) {
				...
				if (messagePxy.groupId) {
					...
					if (typeof contact === "undefined") {
						contact = mckGroupLayout
								.createGroup(messagePxy.groupId);
					}
				} else {
				...
				if (messagePxy.message && FILE_META.length === 0) {
					var isTopPanelAdded = ($mck_tab_message_option
							.hasClass('n-vis'));
					var tabId = $mck_msg_inner.data('mck-id');
					var randomId = mckUtils.randomId();
					messagePxy.key = randomId;
					if (messagePxy.contentType !== 12 && tabId
				...
				$mck_msg_sbmt.attr('disabled', false);
				$applozic("." + randomId + " .mck-message-status").removeClass(
						'mck-icon-sent').addClass('mck-icon-time');
			...
			_this.sendWelcomeMessage = function(params) {
				...
				$applozic.ajax({
					...
					global : false,
					data : "sender=" + encodeURIComponent(params.sender)
							+ "&messageContent="
			...
			_this.submitMessage = function(messagePxy, optns) {
				...
				var $mck_msg_div = $applozic("#mck-message-cell div[name='message']."
						+ randomId);
				$applozic
						.ajax({
							type : "POST",
							...
							contentType : 'application/json',
							success : function(data) {
								var currentTabId = $mck_msg_inner
										.data('mck-id');
								if (typeof data === 'object') {
									...
									if (currentTabId
										...
										$mck_msg_inner.data(
												'mck-conversationid',
												conversationId);
										$mck_msg_div.removeClass(randomId)
												.addClass(messageKey);
										...
										$applozic(
												"."
														+ messageKey
														+ " .mck-message-status")
												.removeClass('mck-icon-time')
												.addClass('mck-icon-sent')
												.attr('title', 'sent');
									...
									$mck_msg_div.remove();
									if (optns.isTopPanelAdded) {
										$mck_tab_message_option.removeClass(
												'vis').addClass('n-vis');
									}
								} else if (data === 'error') {
									$mck_msg_sbmt.attr('disabled', false);
									...
									$mck_msg_error.removeClass('n-vis')
											.addClass('vis');
									$mck_msg_div.remove();
									if (optns.isTopPanelAdded) {
										$mck_tab_message_option.removeClass(
							...
							error : function() {
								$mck_msg_error
										.html('Unable to process your request. Please try again.');
								$mck_msg_error.removeClass('n-vis').addClass(
										'vis');
								if ($mck_msg_div) {
									$mck_msg_div.remove();
								}
								if (optns.isTopPanelAdded) {
									$mck_tab_message_option.removeClass('vis')
											.addClass('n-vis');
								}
							}
						});
			...
			_this.deleteMessage = function(msgKey) {
				...
				if (typeof tabId !== 'undefined') {
					$applozic
							.ajax({
								url : MCK_BASE_URL + MESSAGE_DELETE_URL
										+ "?key=" + msgKey,
								type : 'get',
								success : function(data) {
									if (data === 'success') {
										...
										w.console
												.log('Unable to delete message. Please reload page.');
									}
								}
							});
				}
			...
			_this.deleteConversation = function() {
				...
				if (typeof tabId !== 'undefined') {
					var data = (isGroup) ? "groupId=" + tabId : "userId="
					...
					$applozic
							.ajax({
								...
								success : function() {
									...
									if (currentTabId === tabId) {
										$mck_msg_inner.html("");
										$mck_msg_cell.removeClass('n-vis')
												.addClass('vis');
										$mck_msg_inner
												.html('<div class="mck-no-data-text mck-text-muted">No messages yet!</div>');
										$mck_tab_message_option.removeClass(
												'vis').addClass('n-vis');
									}
									mckStorage.clearMckMessageArray();
			...
			_this.getMessages = function(params) {
				...
				if (typeof params.userId !== "undefined"
						&& params.userId !== "") {
					reqData = (params.isGroup) ? "&groupId=" + params.userId
							: "&userId=" + encodeURIComponent(params.userId);
					if (params.startTime) {
						reqData += "&endTime=" + params.startTime;
					}
					reqData += "&pageSize=30";
					if ((IS_MCK_TOPIC_HEADER || IS_MCK_TOPIC_BOX)
				...
				$applozic.ajax({
					url : MCK_BASE_URL + MESSAGE_LIST_URL + "?startIndex=0"
							+ reqData,
					type : 'get',
					success : function(data) {
			...
			_this.getMessageList = function(params) {
				var tabId = params.id;
				if (typeof params.clientGroupId !== "undefined"
						&& params.clientGroupId !== "") {
					var data = (params.pageSize) ? "&pageSize="
					...
					var data = (params.pageSize) ? "&pageSize="
							+ params.pageSize : "&pageSize=50";
					data += ('' + params.isGroup === 'true') ? "&groupId="
							+ tabId : "&userId=" + tabId;
					if (params.startTime) {
						data += "&endTime=" + params.startTime;
					}
					...
					var data = (params.pageSize) ? "&mainPageSize="
							+ params.pageSize : "&mainPageSize=50";
					if (params.startTime) {
						data += "&endTime=" + params.startTime;
					}
					var resp = {
											...
											.each(
													...
													function(i, message) {
														if (typeof message.to !== "undefined"
															...
															var messageFeed = mckMessageLayout
																	.getMessageFeed(message);
															messageFeeds
																	.push(messageFeed);
														}
													});
			...
			_this.loadMessageList = function(params, callback) {
				...
				if (typeof params.tabId !== "undefined" && params.tabId !== "") {
					reqData = (params.isGroup) ? "&groupId=" + params.tabId
					...
					if ((IS_MCK_TOPIC_HEADER || IS_MCK_TOPIC_BOX)
						...
						if (typeof MCK_TAB_CONVERSATION_MAP[params.tabId] === 'undefined') {
							...
							mckMessageLayout.addConversationMenu(params.tabId,
									params.isGroup);
						}
					}
				...
				$applozic
						.ajax({
							...
							success : function(data) {
								...
								$mck_loading.removeClass('vis').addClass(
										'n-vis');
								$mck_msg_loading.removeClass('vis').addClass(
										'n-vis');
								if (CONTACT_SYNCING) {
									_this.initSearch();
								}
								CONTACT_SYNCING = false;
								if (typeof currTabId === "undefined"
											...
											if (params.startTime) {
												...
												$applozic(
														".mck-message-inner[data-mck-id='"
																...
																+ "']")
														.html(
																'<div class="mck-no-data-text mck-text-muted">No messages yet!</div>');
											}
										} else {
											if (params.startTime) {
												$mck_show_more_icon
												...
												$mck_contacts_inner
														.html('<div class="mck-no-data-text mck-text-muted">No conversations yet!</div>');
											}
											$mck_contacts_inner.data(
													'datetime', "");
										}
									}
									if (data + '' !== "null"
										...
										if (individual) {
																		...
																		if (!IS_MCK_USER_DEACTIVATED) {
																			if (!params.isGroup) {
																				if (userDetail.blockedByThis) {
																					...
																					$mck_tab_title
																							.removeClass('mck-tab-title-w-status');
																					$mck_tab_status
																							.removeClass(
																									'vis')
																							.addClass(
																...
																function(i,
																	...
																	if (typeof conversationPxy === 'object') {
																		...
																		if (conversationPxy.topicDetail) {
																			try {
																				MCK_TOPIC_DETAIL_MAP[conversationPxy.topicId] = $applozic
																						.parseJSON(conversationPxy.topicDetail);
																			} catch (ex) {
																				w.console
																						.log('Incorect Topic Detail!');
																			}
																		}
																	}
																});
											...
											if (!params.startTime) {
												if (params.isGroup) {
													mckGroupLayout
															.addGroupStatus(mckGroupLayout
																	.getGroup(params.tabId));
													mckMessageLayout
															.updateUnreadCount(
																	'group_'
																			+ params.tabId,
														...
														.each(
																...
																function(i,
																	...
																	var contact = mckMessageLayout
																			.getContact(''
																					+ userDetail.userId);
																	(typeof contact === 'undefined') ? mckMessageLayout
																			.createContactWithDetail(userDetail)
																			: mckMessageLayout
																					.updateContactDetail(
																							contact,
																							userDetail);
											...
											if (isMessages) {
												if (params.startTime) {
													mckMessageLayout
															.addContactsFromMessageList(
																	data, false);
													mckStorage
															.updateMckMessageArray(data.message);
												} else {
													mckMessageLayout
															.addContactsFromMessageList(
																	data, true);
													mckStorage
															.setMckMessageArray(data.message);
													$mck_contacts_inner
															.animate({
																scrollTop : 0
															}, 0);
							...
							error : function() {
								...
								$mck_loading.removeClass('vis').addClass(
										'n-vis');
								w.console
										.log('Unable to load messages. Please reload page.');
							}
						});
			};
			_this.updateContactList = function(tabId, isGroup) {
				var tabExpr = (isGroup) ? "groupId=" + tabId : "userId="
						+ encodeURIComponent(tabId);
				...
				$applozic.ajax({
					...
					success : function(data) {
						if (data + '' === "null"
								|| typeof data.message === "undefined"
								|| data.message.length === 0) {
							mckMessageLayout.clearContactMessageData(tabId,
									isGroup);
						} else {
							var message = data.message[0];
							if (typeof message !== 'undefined') {
								(message.groupId) ? mckMessageLayout
										.addGroupFromMessage(message, true)
										: mckMessageLayout
												.addContactsFromMessage(
														message, true);
							}
						}
					},
					error : function() {
						mckMessageLayout
								.clearContactMessageData(tabId, isGroup);
					}
				});
			};
			...
			_this.conversationReadUpdate = function(tabId, isGroup) {
				...
				if (tabId && (mckMessageLayout.getUnreadCount(ucTabId) > 0)) {
					...
					$applozic.ajax({
						...
						success : function() {
							mckMessageLayout
									.updateUnreadCount(ucTabId, 0, true);
						},
						error : function() {
						}
					});
			...
			_this.getConversationId = function(params) {
				$mck_msg_inner = mckMessageLayout.getMckMessageInner();
				if (!params.isGroup && !params.isMessage
				...
				if (params.topicId) {
					...
					if (params.fallBackTemplatesList
								...
								success : function(data) {
									if (typeof data === 'object'
										...
										if (typeof groupPxy === 'object'
											...
											(params.isMessage && conversationPxy.created) ? mckMessageLayout
													.loadTab(
															params,
									...
									if (typeof data === 'object'
										...
										if (typeof conversationPxy === 'object') {
											...
											if (typeof (MCK_PRICE_DETAIL) === "function"
												...
												_this
														.sendConversationCloseUpdate(params.conversationId);
											}
											if (params.messageType
													&& typeof params.message === 'object') {
												var tabId = (params.message.groupId) ? params.message.groupId
														: params.message.to;
					...
					if (params.topicId) {
						...
						if (typeof topicDetail === 'object'
							...
							if (!messagePxy.message) {
								messagePxy.message = $applozic
										.trim(topicDetail.title);
							}
							if (params.conversationId) {
								...
								success : function(data) {
									if (typeof data === 'object'
										...
										if (typeof groupPxy === 'object') {
											...
											params.tabId = group.contactId;
											params.isGroup = true;
											(params.isMessage) ? mckMessageLayout
													.loadTab(
															params,
															_this.dispatchMessage)
													: mckMessageLayout
		...
		function MckContactUtils() {
			...
			_this.formatContactId = function(contactId) {
				...
				return contactId.replace(/\@/g, "AT").replace(/\./g, "DOT")
						.replace(/\*/g, "STAR").replace(/\#/g, "HASH").replace(
								/\|/g, "VBAR").replace(/\+/g, "PLUS").replace(
								/\;/g, "SCOLON").replace(/\?/g, "QMARK");
			};
		}
		function MckMessageLayout() {
			...
			var markup = '<div name="message" class="bubble mck-m-b ${msgKeyExpr} ${msgFloatExpr}" data-msgdelivered="${msgDeliveredExpr}" data-msgsent="${msgSentExpr}" data-msgtype="${msgTypeExpr}" data-msgtime="${msgCreatedAtTime}" data-msgcontent="${replyIdExpr}" data-msgkey="${msgKeyExpr}" data-contact="${toExpr}"><div class="mck-clear"><div class="blk-lg-12"><div class="mck-msg-box ${msgClassExpr}">'
					...
					+ '</div><div class="n-vis mck-context-menu">'
					+ '<ul><li><a class="mck-message-delete">Delete</a></li></ul></div></div>';
			...
			var contactbox = '<li id="li-${contHtmlExpr}" class="person ${contIdExpr}" data-mck-id="${contIdExpr}" data-isgroup="${contTabExpr}" data-mck-conversationid="${conversationExpr}" data-msg-time="${msgCreatedAtTimeExpr}">'
					+ '{{html contImgExpr}}'
					+ '<span class="name">${contNameExpr}</span> '
					+ '<span class="time">${msgCreatedDateExpr}</span>'
					+ '<span class="preview msgTextExpr"></span>'
					+ '<span class="mck-unread-count-box unreadcount ${contUnreadExpr}"><span class="mck-unread-count-text text">{{html contUnreadCount}}</span></span>'
					+ '</li>';
			...
			_this.loadTab = function(params, callback) {
				...
				if (params.tabId) {
					if ($('.person[data-mck-id ="' + params.tabId + '"]').length == 0) {
						_this.updateRecentConversationList(
								params.isGroup ? mckGroupLayout
										.getGroup(params.tabId) : _this
										.fetchContact(params.tabId), undefined,
								true);
					}
					var displayName = params.isGroup ? mckGroupLayout
							.getGroupDisplayName(params.tabId) : _this
							.fetchContact(params.tabId).displayName;
					$('.right .top .name').html(displayName);
					$('.person[data-mck-id ="' + params.tabId + '"]').addClass(
							'active');
					$('.chat[data-mck-id ="' + params.tabId + '"]').addClass(
							'active-chat');
				}

				if ($(".left .person.active").length) {
					$mck_msg_inner = $(".mck-message-inner[data-mck-id='"
							+ params.tabId + "']");
				}
				...
				console.log("currenttabid is: " + currTabId);

				if (currTabId) {
					if ($mck_text_box.html().length > 1
											...
											function() {
												...
												var file = {
													filelb : $fileBox.find(
															'.mck-file-lb')
															.html(),
													filesize : $fileBox.find(
															'.mck-file-sz')
															.html()
												};
												var fileMeta = $fileBox
														.data('mckfile');
				...
				if (params.tabId) {
					...
					if (params.topicId) {
						...
						if (typeof topicDetail === "object") {
							if (IS_MCK_TOPIC_HEADER) {
								...
								$mck_product_box.removeClass('n-vis').addClass(
										'vis');
							}
						}
					}
					if (IS_MCK_LOCSHARE && typeof (w.google.maps) === 'object') {
						$mck_attachfile_box.removeClass("vis")
								.addClass('n-vis');
						$mck_atttachmenu_box.removeClass("n-vis").addClass(
								"vis");
					} else {
						$mck_atttachmenu_box.removeClass("vis").addClass(
								"n-vis");
						$mck_attachfile_box.removeClass("n-vis")
								.addClass('vis');
					}
					var name = _this.getTabDisplayName(params.tabId,
							params.isGroup, params.userName);
					...
					if (MCK_MODE === 'support') {
						$applozic('.mck-tab-link').removeClass('vis').addClass(
								'n-vis');
					}
					...
					mckInitializeChannel.subscibeToTypingChannel(params.tabId,
							params.isGroup);
			...
			_this.processMessageList = function(data, scroll) {
				...
				$applozic(".mck-message-inner.active-chat").data('datetime',
						showMoreDateTime);
				if (!scroll && $scrollToDiv.length > 0) {
					$applozic(".mck-message-inner.active-chat").scrollTop(
			...
			_this.closeConversation = function(data) {
				if (typeof MCK_DISPLAY_TEXT === 'function') {
					...
					if (typeof displayText === 'object') {
						var text = (data === "BUSY_WITH_OTHER") ? displayText.onBusyWithOtherUser
				...
				$mck_msg_error.removeClass('n-vis').addClass('vis').addClass(
						'mck-no-mb');
				$mck_msg_form.removeClass('vis').addClass('n-vis');
			};
			_this.addTooltip = function(msgKey) {
				$applozic("." + msgKey + " .mck-icon-time").attr('title',
						'pending');
				$applozic("." + msgKey + " .mck-btn-trash").attr('title',
						'delete');
				$applozic("." + msgKey + " .mck-icon-sent").attr('title',
						'sent');
				$applozic("." + msgKey + " .mck-btn-forward").attr('title',
						'forward message');
				$applozic("." + msgKey + " .mck-icon-delivered").attr('title',
						'delivered');
				$applozic("." + msgKey + " .mck-icon-read").attr('title',
						'delivered and read');
				$applozic("." + msgKey + " .msgtype-outbox-cr").attr('title',
						'sent via Carrier');
				$applozic("." + msgKey + " .msgtype-outbox-mck").attr('title',
						'sent');
				$applozic("." + msgKey + " .msgtype-inbox-cr").attr('title',
						'received via Carrier');
				$applozic("." + msgKey + " .msgtype-inbox-mck").attr('title',
						'recieved');
			};
			...
			_this.addMessage = function(msg, append, scroll, appendContextMenu) {
				...
				var replyId = msg.key;
				var replyMessageParameters = "'" + msg.deviceKey + "'," + "'"
						+ msg.to + "'" + ",'" + msg.to + "'" + ",'" + replyId
						+ "'";
				var displayName = "";
				var nameTextExpr = "";
				var showNameExpr = "n-vis";
				if (msg.groupId && msg.contentType !== 4
						&& (msg.type === 0 || msg.type === 4 || msg.type === 6)) {
					var displayName = _this.getTabDisplayName(msg.to, false);
					showNameExpr = "vis";
					nameTextExpr = _this
							.getNameTextClassByAlphabet(displayName);
				}
				...
				append ? $applozic.tmpl("messageTemplate", msgList).appendTo(
						"#mck-message-cell .mck-message-inner-right")
						: $applozic.tmpl("messageTemplate", msgList).prependTo(
								"#mck-message-cell .mck-message-inner-right");
				append ? $applozic.tmpl("messageTemplate", msgList).appendTo(
						".mck-message-inner[data-mck-id='" + tabId + "']")
						: $applozic.tmpl("messageTemplate", msgList).prependTo(
								".mck-message-inner[data-mck-id='" + tabId
										+ "']");
				...
				if (msg.contentType === 4) {
					...
					if (!MCK_PRICE_WIDGET_ENABLED)
						emoji_template += '<br/><button class="mck-accept" data-mck-topic-price="'
								+ priceText
								+ '" data-mck-conversationid="'
								+ msg.conversationId + '">Accept</button>';
				}
				var $textMessage = $applozic("." + replyId
						+ " .mck-msg-content");
				...
				if (msg.fileMeta) {
					$applozic("." + replyId + " .mck-file-text a").trigger(
							'click');
					$applozic("." + replyId + " .mck-file-text").removeClass(
							'n-vis').addClass('vis');
					if ($textMessage.html() === "") {
				...
				if ($mck_tab_message_option.hasClass('n-vis')) {
					$mck_tab_message_option.removeClass('n-vis')
							.addClass('vis');
				}
				...
				if (msg.contentType === 2) {
					try {
						...
						if (geoLoc.lat && geoLoc.lon) {
							return '<a href="http://maps.google.com/maps?z=17&t=m&q=loc:'
									+ geoLoc.lat
									+ ","
									+ geoLoc.lon
				...
				if (typeof msg.fileMeta === "object") {
					if (msg.fileMeta.contentType.indexOf("image") !== -1) {
						if (msg.fileMeta.contentType.indexOf("svg") !== -1) {
							return '<a href="#" role="link" class="file-preview-link fancybox-media fancybox" data-type="'
						...
						return '<a href="#" role="link" class="file-preview-link fancybox-media fancybox" data-type="'
								...
								+ '" data-name="'
								+ msg.fileMeta.name
								+ '"><div class="mck-video-box n-vis"><video controls preload><source src="'
								+ MCK_FILE_URL
								+ FILE_PREVIEW_URL
								+ msg.fileMeta.blobKey
								+ '" type="'
								+ msg.fileMeta.contentType
								+ '">Your browser does not support the HTML 5 video tag</video></div><span class="file-detail"><span class="mck-file-name"><span class="mck-icon-attachment"></span>&nbsp;'
								+ msg.fileMeta.name
								+ '</span>&nbsp;<span class="file-size">'
								+ mckFileService
										.getFilePreviewSize(msg.fileMeta.size)
								+ '</span></span></a>';
					} else {
						return '<a href="'
								+ MCK_FILE_URL
								+ FILE_PREVIEW_URL
								+ msg.fileMeta.blobKey
								+ '" role="link" class="file-preview-link" target="_blank"><span class="file-detail"><span class="mck-file-name"><span class="mck-icon-attachment"></span>&nbsp;'
								+ msg.fileMeta.name
								+ '</span>&nbsp;<span class="file-size">'
								+ mckFileService
										.getFilePreviewSize(msg.fileMeta.size)
			...
			_this.getFileIcon = function(msg) {
				if (msg.fileMetaKey && typeof msg.fileMeta === "object") {
					return (msg.fileMeta.contentType.indexOf("image") !== -1) ? '<span class="mck-icon-camera"></span>&nbsp;<span>image</span>'
							: '<span class="mck-icon-attachment"></span>&nbsp;<span>file</span>';
				} else {
					return "";
				}
			};
			_this.getContactImageLink = function(contact, displayName) {
				var imgsrctag = "";
				if (!displayName) {
					displayName = mckGroupLayout
							.getGroupDisplayName(contact.contactId);
				}
				if (contact.isGroup) {
					...
					if (!imgsrctag) {
						if (contact.photoSrc) {
							...
							imgsrctag = '<img src="' + MCK_BASE_URL
							...
							imgsrctag = _this
									.getContactImageByAlphabet(displayName);
						}
					}
				}
				return imgsrctag;
			...
			_this.getContactImageByAlphabet = function(name) {
				...
				if (first_alpha.match(letters)) {
					...
					return '<div class="mck-alpha-contact-image alpha_'
							+ first_alpha + '"><span class="mck-contact-icon">'
							+ first_alpha + '</span></div>';
				} else {
			...
			_this.addContactsFromMessageList = function(data, isReloaded) {
				...
				if (data + '' === "null") {
					...
					if (isReloaded) {
						$applozic("#mck-message-cell .mck-message-inner-left")
								.html(
					...
					if (typeof data.message.length === "undefined") {
						...
						$applozic.each(data.message, function(i, message) {
							if (!(typeof message.to === "undefined")) {
								(message.groupId) ? _this.addGroupFromMessage(
										message, true) : _this
										.addContactsFromMessage(message, true);
								showMoreDateTime = message.createdAtTime;
							}
						});
					}
			...
			_this.addGroupFromMessageList = function(data, isReloaded) {
				if (data + '' === "null") {
					...
					if (isReloaded) {
						$mck_contacts_inner
								.html('<ul id="mck-group-list" class="mck-contact-list mck-nav mck-nav-tabs mck-nav-stacked"></ul>');
					}
					if (typeof data.message.length === "undefined") {
						_this.addGroupFromMessage(data.message);
					} else {
				...
				if (contactIdsArray.length > 0 && contactIdsArray[0]) {
					for (var i = 0; i < contactIdsArray.length; i++) {
						var contact = _this.fetchContact(''
								+ contactIdsArray[i]);
						_this.updateRecentConversationList(contact, message,
								update);
					}
				}
			};
			...
			_this.updateRecentConversationList = function(contact, message,
					update) {
				var $listId = "mck-contact-list";
				var contactHtmlExpr = (contact.isGroup) ? 'group-'
						+ contact.htmlId : 'user-' + contact.htmlId;
				if ($applozic("#" + $listId + " #li-" + contactHtmlExpr).length > 0) {
					var $mck_msg_part = $applozic("#" + $listId + " #li-"
							+ contact.htmlId + " .mck-cont-msg-wrapper");
					if (($mck_msg_part.is(":empty") || update)
							&& message !== undefined) {
			...
			_this.addContactsToSearchList = function(userIdArray, isLocal) {
				...
				for (var j = 0; j < userIdArray.length; j++) {
					...
					if (userId) {
						...
						if ($applozic("#mck-search-list #li-user-"
								+ contact.htmlId).length === 0) {
			...
			_this.initAutoSuggest = function(params) {
				...
				for (var j = 0; j < contactsArray.length; j++) {
					...
					typeaheadEntry = (contact.displayName) ? $applozic
							.trim(contact.displayName) : $applozic
							.trim(contact.contactId);
				...
				$searchId
						.mcktypeahead({
							...
							matcher : function(item) {
								...
								var contactNameArray = contact.displayName
										.split(" ");
			...
			_this.updateContact = function(contact, message, $listId) {
				var contHtmlExpr = (contact.isGroup) ? 'group-'
						+ contact.htmlId : 'user-' + contact.htmlId;
				...
				if (message && message.createdAtTime > currentMessageTime) {
					var ucTabId = (message.groupId) ? 'group_'
							+ contact.contactId : 'user_' + contact.contactId;
					var unreadCount = _this.getUnreadCount(ucTabId);
					var emoji_template = _this.getMessageTextForContactPreview(
							message, contact, 15);
					$applozic("#li-" + contHtmlExpr + " .mck-cont-msg-date")
							.html(
											...
											: mckDateUtils
													.getTimeOrDate(
															message ? message.createdAtTime
																	: "", true));
					var $messageText = $applozic("#li-" + contHtmlExpr
							+ " .mck-cont-msg-wrapper");
					$messageText.html("");
					(typeof emoji_template === 'object') ? $messageText
							.append(emoji_template) : $messageText
							.html(emoji_template);
					if (message.conversationId) {
						...
						$applozic("#li-" + contHtmlExpr + " a").data(
								'mck-conversationid', conversationId);
					}
					if (unreadCount > 0) {
						$applozic(
								"#li-" + contHtmlExpr
										+ " .mck-unread-count-text").html(
								unreadCount);
						$applozic(
								"#li-" + contHtmlExpr
					...
					if ((typeof latestCreatedAtTime === "undefined" || (message ? message.createdAtTime
							: "") >= latestCreatedAtTime)
			...
			_this.clearContactMessageData = function(tabId, isGroup) {
				...
				var contactIdExpr = (isGroup) ? 'group-' + htmlId : 'user-'
						+ htmlId;
				$applozic("#li-" + contactIdExpr + " .mck-cont-msg-date").html(
						"");
				$applozic("#li-" + contactIdExpr + " .mck-cont-msg-wrapper")
						.html("");
			};
			_this.addContact = function(contact, $listId, message) {
				...
				var displayName = _this.getTabDisplayName(contact.contactId,
						isGroupTab);
				var imgsrctag = _this.getContactImageLink(contact, displayName);
				var prepend = false;
				var ucTabId = (isGroupTab) ? 'group_' + contact.contactId
						: 'user_' + contact.contactId;
				var unreadCount = _this.getUnreadCount(ucTabId);
				var unreadCountStatus = (unreadCount > 0 && $listId !== "mck-search-list") ? "vis"
						: "n-vis";
				var olStatus = "n-vis";
				if (!isGroupTab && !MCK_BLOCKED_TO_MAP[contact.contactId]
						&& !MCK_BLOCKED_BY_MAP[contact.contactId]
						&& IS_MCK_OL_STATUS && w.MCK_OL_MAP[contact.contactId]) {
					olStatus = "vis";
					prepend = true;
				}
				...
				var contHtmlExpr = (isGroupTab) ? 'group-' + contact.htmlId
						: 'user-' + contact.htmlId;
				var contactList = [ {
					...
					titleExpr : title,
					msgCreatedDateExpr : message ? mckDateUtils.getTimeOrDate(
							message.createdAtTime, true) : ""
				} ];
				var latestCreatedAtTime = $applozic(
						'#' + $listId + ' li:nth-child(1)').data('msg-time');
				if (typeof latestCreatedAtTime === "undefined"
						|| (message ? message.createdAtTime : "") > latestCreatedAtTime
						|| ($listId.indexOf("search") !== -1 && prepend)) {
					$applozic.tmpl("contactTemplate", contactList).prependTo(
							'#' + $listId);
					if ($listId !== "mck-search-list") {
						$applozic.tmpl("conversationTemplate", contactList)
								.prependTo('#conversation-section');
					}
				} else {
					$applozic.tmpl("contactTemplate", contactList).appendTo(
							'#' + $listId);
					if ($listId !== "mck-search-list") {
						$applozic.tmpl("conversationTemplate", contactList)
								.appendTo('#conversation-section');
					}
				}
				var $textMessage = $applozic("#li-" + contHtmlExpr
						+ " .msgTextExpr");
				(typeof emoji_template === 'object') ? $textMessage
						.append(emoji_template) : $textMessage
						.html(emoji_template);
				if (!$(".left .person").length) {
					_this.loadTab({
						tabId : isGroupTab ? message.groupId
								: contact.contactId,
						'isGroup' : isGroupTab
					});
				}
			...
			_this.addConversationMenu = function(tabId, isGroup) {
				...
				var tabConvArray = MCK_TAB_CONVERSATION_MAP[tabId];
				if (typeof tabConvArray === 'undefined'
				...
				$applozic
						.each(
								tabConvArray,
								function(i, convPxy) {
									if ($applozic("#mck-conversation-list #li-"
											+ convPxy.id).length === 0) {
										var title = "";
										if (convPxy.topicDetail) {
											var topicDetail = $applozic
													.parseJSON(convPxy.topicDetail);
											title = (typeof topicDetail === 'object') ? topicDetail.title
													: convPxy.topicDetail;
										}
										if (!title) {
			...
			_this.getStatusIcon = function(msg) {
				return '<span class="' + _this.getStatusIconName(msg)
						+ ' move-right ' + msg.key
						+ '_status status-icon"></span>';
			};
			_this.getStatusIconName = function(msg) {
				if (msg.type === 7 || msg.type === 6 || msg.type === 4
						|| msg.type === 0) {
					return "";
				}
				if (msg.status === 5) {
			...
			_this.clearMessageField = function() {
				$mck_text_box.html("");
				$mck_msg_sbmt.attr('disabled', false);
				$mck_file_box.removeClass('vis').removeClass('mck-text-req')
						.addClass('n-vis').attr("required", "").html("");
			};
			_this.addDraftMessage = function(tabId) {
				...
				if (tabId && typeof TAB_MESSAGE_DRAFT[tabId] === 'object') {
					...
					if (draftMessage.files.length > 0) {
						...
						$mck_file_box.removeClass('n-vis').removeClass(
								'mck-text-req').addClass('vis').removeAttr(
								'required');
					}
				} else {
					FILE_META = [];
			...
			_this.removedDeletedMessage = function(key, tabId, isGroup) {
				...
				if ($divMessage.length > 0) {
					...
					if ($mck_msg_inner.is(":empty")) {
						$mck_msg_cell.removeClass('n-vis').addClass('vis');
						$mck_tab_message_option.removeClass('vis').addClass(
								'n-vis');
					}
				...
				if (typeof message !== 'undefined') {
					if (message.message) {
						...
						if (mckUtils.startsWith(msg, "<img")) {
							...
							emoji_template = w.emoji
									.replace_colons(emoji_template);
							emoji_template = (emoji_template.indexOf('</span>') !== -1) ? emoji_template
									.substring(0, emoji_template
											.lastIndexOf('</span>'))
									: emoji_template.substring(0, size);
						}
						if (!contact.isGroup) {
							if (emoji_template.indexOf('emoji-inner') === -1
					...
					} else if (message.fileMetaKey
							&& typeof message.fileMeta === "object") {
						emoji_template = (message.fileMeta.contentType
								.indexOf("image") !== -1) ? '<span class="mck-icon-camera"></span>&nbsp;<span>image</span>'
								: '<span class="mck-icon-attachment"></span>&nbsp;<span>file</span>';
					}
					if (contact.isGroup && contact.type !== 3) {
						var msgFrom = (message.to.split(",")[0] === MCK_USER_ID) ? "Me"
								: mckMessageLayout.getTabDisplayName(message.to
										.split(",")[0], false);
				...
				if (typeof message !== 'undefined') {
					if (message.message) {
						...
						if (mckUtils.startsWith(msg, "<img")) {
							...
							&& typeof message.fileMeta === "object") {
						emoji_template = (message.fileMeta.contentType
								.indexOf("image") !== -1) ? 'image attachment'
								: 'file attachment';
					}
					if (contact.isGroup && contact.type !== 3) {
						var msgFrom = (message.to.split(",")[0] === MCK_USER_ID) ? "Me"
			...
			_this.getUserIdArrayFromMessageList = function(messages) {
				...
				if (typeof messages.length === "undefined") {
					...
					$applozic.each(messages, function(i, message) {
						if (!(typeof message.to === "undefined")) {
							userIdArray = userIdArray.concat(_this
			...
			_this.messageContextMenu = function(messageKey) {
				...
				if ($messageBox.addEventListener) {
					...
					$messageBox.bind('contextmenu', function(e) {
						e.preventDefault();
						$applozic(".mck-context-menu").removeClass("vis")
								.addClass("n-vis");
						$applozic("." + messageKey + " .mck-context-menu")
								.removeClass("n-vis").addClass("vis");
			...
			_this.updateDraftMessage = function(tabId, fileMeta) {
				if (typeof fileMeta === 'object') {
					...
					var file = {
						...
						filesize : mckFileService
								.getFilePreviewSize(fileMeta.size)
					};
					if ((typeof tabId !== 'undefined')
			...
			_this.updateUnreadCount = function(tabId, count, isTotalUpdate) {
				...
				if (isTotalUpdate && $mckChatLauncherIcon.length > 0) {
					...
					(MCK_TOTAL_UNREAD_COUNT > 0) ? $mckChatLauncherIcon
							.html(MCK_TOTAL_UNREAD_COUNT)
							: $mckChatLauncherIcon.html("");
				}
			};
			_this.incrementUnreadCount = function(tabId) {
				MCK_TOTAL_UNREAD_COUNT += 1;
				MCK_UNREAD_COUNT_MAP[tabId] = (typeof (MCK_UNREAD_COUNT_MAP[tabId]) === 'undefined') ? 1
			...
			_this.populateMessage = function(messageType, message, notifyUser) {
				$mck_msg_inner = mckMessageLayout.getMckMessageInner();
				var tabId = $mck_msg_inner.data('mck-id');
				var contact = (message.groupId) ? mckGroupLayout
						.getGroup(message.groupId) : mckMessageLayout
						.getContact(message.to);
				if ((typeof tabId === "undefined") || tabId === "") {
					var mckContactListLength = $applozic("#mck-contact-list").length;
					if (mckContactListLength > 0) {
						(message.groupId) ? mckMessageLayout
					...
					if (messageType === "APPLOZIC_01"
						...
						if (typeof contact === 'undefined') {
							contact = (message.groupId) ? mckGroupLayout
									.createGroup(message.groupId)
									: mckMessageLayout
											.createContact(message.to);
						}
						var ucTabId = (message.groupId) ? 'group_'
								+ contact.contactId : 'user_'
								+ contact.contactId;
						if (message.contentType !== 10) {
							mckMessageLayout.incrementUnreadCount(ucTabId);
						}
						mckNotificationService.notifyUser(message);
						var contactHtmlExpr = (message.groupId) ? 'group-'
								+ contact.htmlId : 'user-' + contact.htmlId;
						$applozic(
								"#li-" + contactHtmlExpr
										+ " .mck-unread-count-text").html(
					...
					if (messageType === "APPLOZIC_01"
						...
						if (typeof contact !== 'undefined') {
							...
							if (typeof tabId !== 'undefined'
								...
								if (message.conversationId
									...
									var currConvId = $mck_msg_inner
											.data('mck-conversationid');
									if (currConvId
											&& currConvId.toString() === message.conversationId
													.toString()) {
										mckMessageLayout.addMessage(message,
												true, true, true);
										mckMessageService
												.sendReadUpdate(message.pairedMessageKey);
									}
								} else {
						...
						if (($applozic("." + message.oldKey).length === 0 && $applozic("."
			...
			_this.getMessageFeed = function(message) {
				...
				messageFeed.from = (message.type === 4) ? message.to
						: MCK_USER_ID;
				if (message.groupId) {
					messageFeed.to = message.groupId;
				} else {
					messageFeed.to = (message.type === 5) ? message.to
							: MCK_USER_ID;
				}
				...
				if (typeof message.fileMeta === 'object') {
					...
					file.url = MCK_FILE_URL + '/rest/ws/aws/file/'
				...
				if (IS_LAUNCH_ON_UNREAD_MESSAGE_ENABLED) {
					...
					if (data.users.length > 0) {
						...
						if (MCK_TOTAL_UNREAD_COUNT > 0
							...
							if (contactIdWithUnreadMessage !== null
		...
		function MckUserUtils() {
			...
			_this.checkUserConnectedStatus = function() {
				...
				$applozic(".mck-user-ol-status").each(
						function() {
				...
				if (userIdArray.length > 0) {
					...
					(otherUserIdArray.length > 0) ? mckContactService
							.getUsersDetail(otherUserIdArray, {
								setStatus : true
							}) : _this.updateUserConnectedStatus();
				}
			};
			_this.updateUserConnectedStatus = function() {
				$applozic(".mck-user-ol-status")
						.each(
								function() {
									var $this = $applozic(this);
									var tabId = $this.data('mck-id');
									if (tabId) {
										...
										if (typeof MCK_USER_DETAIL_MAP[tabId] !== 'undefined'
												&& userDetail.connected) {
											$this.removeClass('n-vis')
													.addClass('vis');
											$this.next().html('(Online)');
										} else {
											$this.removeClass('vis').addClass(
													'n-vis');
				...
				if (isBlocked) {
					...
					if (!MCK_BLOCKED_BY_MAP[tabId]
						...
						if (w.MCK_OL_MAP[tabId]) {
							...
							var lastSeenAt = mckDateUtils
									.getLastSeenAtStatus(MCK_LAST_SEEN_AT_MAP[tabId]);
		...
		function MckContactService() {
			...
			_this.getContactDisplayName = function(userIdArray) {
				...
				if (userIdArray.length > 0 && userIdArray[0]) {
					var data = "";
					var uniqueUserIdArray = userIdArray.filter(function(item,
							pos) {
						...
						$applozic
								.ajax({
									...
									success : function(data) {
										...
										mckStorage
												.updateMckContactNameArray(mckContactNameArray);
									},
									error : function() {
									}
								});
			...
			_this.loadContacts = function() {
				...
				$applozic
						.ajax({
							url : MCK_BASE_URL + CONTACT_LIST_URL
									+ "?startIndex=0&pageSize=30&orderBy=1",
							type : 'get',
							global : false,
							success : function(data) {
								$mck_search_loading.removeClass('vis')
										.addClass('n-vis');
								if ($mck_sidebox_search.length == 0
										|| $mck_sidebox_search.hasClass('vis')) {
														...
														function(i, user) {
															if (typeof user.userId !== "undefined") {
																...
																var contact = mckMessageLayout
																		.getContact(''
																				+ user.userId);
																contact = (typeof contact === 'undefined') ? mckMessageLayout
																		.createContactWithDetail(user)
																		: mckMessageLayout
																				.updateContactDetail(
																						contact,
																						user);
							...
							error : function() {
								$mck_search_loading.removeClass('vis')
										.addClass('n-vis');
								w.console
										.log('Unable to load contacts. Please reload page.');
							}
						});
			};
						...
						.ajax({
							...
							error : function() {
								$mck_loading.removeClass('vis').addClass(
										'n-vis');
								w.console
										.log('Unable to load groups. Please reload page.');
								if (!isInternal) {
									response.status = "error";
									if (params.callback) {
										params.callback(response);
						...
						.ajax({
							...
							error : function() {
								$mck_loading.removeClass('vis').addClass(
										'n-vis');
								w.console
										.log('Unable to load group. Please reload page.');
								if (!isInternal) {
									response.status = "error";
						...
						.ajax({
							...
							success : function(data) {
								if (data.status === "success") {
									...
									if (params.groupId) {
										mckGroupLayout.onAddedGroupMember(
												params.groupId, params.userId);
									}
									return;
								} else {
		...
		function MckGroupLayout() {
			...
			var groupContactbox = '<li id="li-${contHtmlExpr}" class="${contIdExpr} mck-li-group-member" data-mck-id="${contIdExpr}" data-alpha="${contFirstAlphaExpr}">'
					...
					+ '<div class="blk-lg-4 mck-group-admin-text move-right ${isAdminExpr}"><span>Admin</span></div></div>'
					+ '<div class="mck-row">'
					+ '<div class="blk-lg-10 mck-truncate mck-last-seen-status" title="${contLastSeenExpr}">${contLastSeenExpr}</div>'
					+ '<div class="move-right blk-lg-2 mck-group-admin-options ${enableAdminMenuExpr}">'
					...
					+ '<a href="#" class="mck-btn-remove-member menu-item" title="Remove member">Remove member</a>'
					+ '</li></ul></div></div>' + '</div></div></div></li>';
			var groupSearchContact = '<li id="li-${contHtmlExpr}" class="${contIdExpr} mck-li-group-member" data-mck-id="${contIdExpr}">'
				...
				if (typeof MCK_GROUP_MAP[groupId] === 'object') {
					...
					if (group.type === 3) {
						if (displayName.indexOf(MCK_USER_ID) !== -1) {
							displayName = displayName.replace(MCK_USER_ID, "")
									.replace(":", "");
							if (typeof (MCK_GETUSERNAME) === "function") {
			...
			_this.getGroupImage = function(imageSrc) {
				return (imageSrc) ? '<img src="' + imageSrc + '"/>'
						: '<img src="'
								+ MCK_BASE_URL
								+ '/resources/sidebox/css/app/images/mck-icon-group.png"/>';
			};
			...
			_this.addGroupStatus = function(group) {
				...
				if (isGroupLeft) {
					...
					$mck_tab_status.removeClass('n-vis').addClass('vis');
					$mck_tab_title.addClass("mck-tab-title-w-status");
					$mck_group_menu_options.removeClass('n-vis')
							.addClass('vis');
				} else {
					$mck_tab_title.removeClass("mck-tab-title-w-status");
					$mck_tab_status.removeClass('vis').addClass('n-vis');
			...
			_this.isGroupLeft = function(group) {
				...
				if (group.removedMembersId && group.removedMembersId.length > 0) {
					$applozic.each(group.removedMembersId, function(i,
							removedMemberId) {
			...
			_this.onGroupLeft = function(groupId) {
				if ($mck_group_info_tab.hasClass('vis')) {
					...
					if (currTabId === groupId.toString() && isGroupTab) {
						$mck_group_menu_options.removeClass('vis').addClass(
								'n-vis');
				...
				if (typeof group === 'object') {
					...
					if ($mck_group_info_tab.hasClass('vis')) {
						...
						if (groupId === currGroupId) {
							var contact = mckMessageLayout.fetchContact(''
									+ userId);
							if ($applozic("#mck-group-member-list #li-user-"
									+ contact.htmlId).length === 0) {
				...
				if (typeof group === 'object') {
					...
					if ($mck_group_info_tab.hasClass('vis')) {
						...
						if (groupId === currGroupId) {
							var contact = mckMessageLayout.fetchContact(''
									+ userId);
							var $liRemovedMember = $applozic("#li-user-"
									+ contact.htmlId);
			...
			_this.enableGroupAdminMenuToggle = function() {
				$applozic('.mck-group-member-info').bind(
						"mouseenter",
						function() {
							$applozic(this).find('.mck-menu-box').removeClass(
									'n-vis');
						}).bind(
						"mouseleave",
						function() {
			...
			_this.addGroupMember = function(group, contact) {
				...
				if (!MCK_BLOCKED_TO_MAP[contact.contactId]) {
					if (w.MCK_OL_MAP[contact.contactId]) {
						...
						lastSeenStatus = mckDateUtils
								.getLastSeenAtStatus(MCK_LAST_SEEN_AT_MAP[contact.contactId]);
					}
				}
				...
				$applozic.tmpl("groupMemberTemplate", contactList).appendTo(
						'#mck-group-member-list');
			};
			_this.addMembersToGroupSearchList = function() {
				...
				$applozic.each(contactArray, function(i, userId) {
					if (userId) {
						var contact = mckMessageLayout
								.fetchContact('' + userId);
						if ($applozic("#mck-group-search-list #li-user-"
								+ contact.htmlId).length === 0) {
			...
			_this.addGroupSearchMember = function(contact) {
				...
				$applozic.tmpl("groupSearchTemplate", contactList).appendTo(
			...
			_this.loadGroupInfo = function(params) {
				if (params.groupId) {
					...
					if (params.conversationId) {
						$mck_group_info_tab.data('mck-conversation-id',
								params.conversationId);
					}
					$mck_group_member_List.html('');
					var group = _this.getGroup(params.groupId);
					if (typeof group === 'object') {
						$mck_group_icon.html(_this
								.getGroupImage(group.imageSrc));
						$mck_group_title.html(group.displayName);
						_this.addMembersToGroupInfoList(group);
						(group.adminName === MCK_USER_ID) ? $mck_group_add_member_box
								.removeClass('n-vis').addClass('vis')
								: $mck_group_add_member_box.removeClass('vis')
										.addClass('n-vis');
					} else {
						mckGroupService.getGroupFeed({
			...
			_this.sortGroupMemberHtmlList = function() {
				$applozic('#mck-group-member-list .mck-li-group-member').sort(
						function(a, b) {
			...
			_this.addGroupMemberFromSearch = function(userId) {
				...
				if (typeof groupId !== 'undefined'
					...
					if (typeof group === 'object'
						...
						$mck_group_admin_options.removeClass('vis').addClass(
								'n-vis');
					}
				}
		...
		function MckStorage() {
			...
			_this.getMckMessageArray = function() {
				return (typeof (w.sessionStorage) !== "undefined") ? $applozic
						.parseJSON(w.sessionStorage.getItem("mckMessageArray"))
						: MCK_MESSAGE_ARRAY;
			};
			...
			_this.setMckMessageArray = function(messages) {
				if (typeof (w.sessionStorage) !== "undefined") {
					w.sessionStorage.setItem('mckMessageArray', w.JSON
							.stringify(messages));
				} else {
			...
			_this.updateMckMessageArray = function(mckMessageArray) {
				if (typeof (w.sessionStorage) !== "undefined") {
					var mckLocalMessageArray = $applozic
							.parseJSON(w.sessionStorage
									.getItem('mckMessageArray'));
					if (mckLocalMessageArray !== null) {
						mckLocalMessageArray = mckLocalMessageArray
								.concat(mckMessageArray);
						w.sessionStorage.setItem('mckMessageArray', w.JSON
								.stringify(mckLocalMessageArray));
					} else {
						w.sessionStorage.setItem('mckMessageArray', w.JSON
								.stringify(mckMessageArray));
					}
					return mckMessageArray;
				} else {
					MCK_MESSAGE_ARRAY = MCK_MESSAGE_ARRAY
							.concat(mckMessageArray);
			...
			_this.getMckContactNameArray = function() {
				return (typeof (w.sessionStorage) !== "undefined") ? $applozic
						.parseJSON(w.sessionStorage
								.getItem("mckContactNameArray"))
						: MCK_CONTACT_NAME_ARRAY;
			};
			_this.setMckContactNameArray = function(mckContactNameArray) {
				if (typeof (w.sessionStorage) !== "undefined") {
					w.sessionStorage.setItem('mckContactNameArray', w.JSON
							.stringify(mckContactNameArray));
				} else {
			...
			_this.updateMckContactNameArray = function(mckContactNameArray) {
				if (typeof (w.sessionStorage) !== "undefined") {
					var mckLocalcontactNameArray = $applozic
							.parseJSON(w.sessionStorage
									.getItem('mckContactNameArray'));
					if (mckLocalcontactNameArray !== null) {
						mckContactNameArray = mckContactNameArray
								.concat(mckLocalcontactNameArray);
					}
					w.sessionStorage.setItem('mckContactNameArray', w.JSON
							.stringify(mckContactNameArray));
					return mckContactNameArray;
				} else {
					MCK_CONTACT_NAME_ARRAY = MCK_CONTACT_NAME_ARRAY
							.concat(mckContactNameArray);
		...
		function MckMapLayout() {
			...
			_this.init = function() {
				if (IS_MCK_LOCSHARE && typeof (w.google.maps) === 'object') {
					...
					_this.getCurrentLocation(_this.onGetCurrLocation,
							_this.onErrorCurrLocation);
				}
				$mck_my_loc.on("click", function() {
					_this.getCurrentLocation(_this.onGetMyCurrLocation,
			...
			_this.onErrorMyCurrLocation = function(err) {
				alert("Unable to retrieve your location. ERROR(" + err.code
						+ "): " + err.message);
			};
		...
		function MckMapService() {
			...
			$mck_loc_submit.on("click", function() {
				var messagePxy = {
					"type" : 5,
					"contentType" : 2,
					"message" : w.JSON.stringify(mckMapLayout
							.getSelectedLocation())
				};
				var conversationId = $mck_msg_inner.data('mck-conversationid');
		...
		function MckFileService() {
			...
			var mck_filebox_tmpl = '<div id="mck-filebox-${fileIdExpr}" class="mck-file-box ${fileIdExpr}">'
					...
					+ '<span class="mck-file-content blk-lg-7"><span class="mck-file-lb">{{html fileNameExpr}}</span>&nbsp;<span class="mck-file-sz">${fileSizeExpr}</span></span>'
					...
					+ '<span class="move-right blk-lg-2">'
			...
			_this.init = function() {
						...
						.on(
								...
								function() {
									...
									if ($applozic(".mck-file-box").length > 4) {
										uploadErrors
												.push("Can't upload more than 5 files at a time");
									}
									if (file['size'] > (MCK_FILEMAXSIZE * ONE_MB)) {
										uploadErrors
												.push("file size can not be more than "
														+ MCK_FILEMAXSIZE
														+ " MB");
									}
									if (uploadErrors.length > 0) {
										...
										var fileboxList = [ {
											...
											fileNameExpr : '<a href="#">'
													+ file.name + '</a>',
											fileSizeExpr : _this
													.getFilePreviewSize(file.size)
										} ];
										$applozic.tmpl("fileboxTemplate",
												fileboxList).appendTo(
												'#mck-file-box');
										var $fileContainer = $applozic(".mck-file-box."
												+ randomId);
										var $file_name = $applozic(".mck-file-box."
												+ randomId + " .mck-file-lb");
										var $file_progressbar = $applozic(".mck-file-box."
												+ randomId + " .progress .bar");
										var $file_progress = $applozic(".mck-file-box."
												+ randomId + " .progress");
										var $file_remove = $applozic(".mck-file-box."
												+ randomId
												+ " .mck-remove-file");
										$file_progressbar.css('width', '0%');
										$file_progress.removeClass('n-vis')
												.addClass('vis');
										$file_remove.attr("disabled", true);
										$mck_file_upload.attr("disabled", true);
										$file_box.removeClass('n-vis')
												.addClass('vis');
										if (file.name === $applozic(
												".mck-file-box." + randomId
														+ " .mck-file-lb a")
												.html()) {
											var currTab = $mck_msg_inner
													.data('mck-id');
											var uniqueId = file.name
													+ file.size;
				...
				$applozic(d)
						.on(
								...
								'.mck-remove-file',
								function() {
									var $currFileBox = $applozic(this).parents(
											'.mck-file-box');
									var currFileMeta = $currFileBox
											.data('mckfile');
									$currFileBox.remove();
									$mck_msg_sbmt.attr('disabled', false);
									if ($file_box.find('.mck-file-box').length === 0) {
										$file_box.removeClass('vis').addClass(
												'n-vis');
										$mck_text_box.attr("required", "");
									}
									if (typeof currFileMeta === 'object') {
										_this
												.deleteFileMeta(currFileMeta.blobKey);
										$applozic
												.each(
														FILE_META,
														function(i, fileMeta) {
			...
			_this.getFilePreviewPath = function(fileMeta) {
				return (typeof fileMeta === "object") ? '<a href="'
						+ MCK_FILE_URL + FILE_PREVIEW_URL + fileMeta.blobKey
						+ '" target="_blank">' + fileMeta.name + '</a>' : "";
			};
			...
			_this.addFileBox = function(file) {
				...
				$applozic.tmpl("fileboxTemplate", fileboxList).appendTo(
						'#mck-file-box');
		...
		function MckNotificationService() {
			...
			_this.notifyUser = function(message) {
				...
				var contact = (message.groupId) ? mckGroupLayout.getGroup(''
						+ message.groupId) : mckMessageLayout.fetchContact(''
						+ message.to.split(",")[0]);
				var isGroup = false;
				if (message.groupId) {
					isGroup = true;
				...
				if (IS_MCK_NOTIFICATION && !IS_MCK_TAB_FOCUSED) {
					if (_this.isChrome()) {
						var msg = mckMessageLayout.getTextForMessagePreview(
								message, contact);
						var c_version = parseInt(w.navigator.appVersion
								.match(/Chrome\/(\d+)\./)[1], 10);
						if (c_version >= 35) {
							if (!w.Notification) {
								return;
							...
							if (w.webkitNotifications.checkPermission() === 0) {
								var notification = w.webkitNotifications
										.createNotification(iconLink,
												displayName, msg);
						...
						if (w.webkitNotifications.checkPermission() === 0) {
							var notification = w.webkitNotifications
									.createNotification(iconLink, displayName,
											msg);
							_this.showNotification(notification);
						}
					}
			...
			_this.showNewMessageNotification = function(message, contact,
					displayName) {
				...
				if (currTabId === contact.contactId
					...
					if (message.conversationId
						...
						var currConvId = $mck_msg_inner
								.data('mck-conversationid');
						currConvId = (typeof currConvId !== "undefined" && currConvId !== "") ? currConvId
								.toString()
								: "";
				...
				if (message.message) {
					...
					(typeof msg === 'object') ? $mck_preview_msg_content
							.append(msg) : $mck_preview_msg_content.html(msg);
					$mck_preview_msg_content.removeClass('n-vis').addClass(
							'vis');
				} else {
					$mck_preview_msg_content.html("");
				}
				if (message.fileMetaKey) {
					$mck_preview_file_content.html(mckMessageLayout
							.getFileIcon(message));
					$mck_preview_file_content.removeClass('n-vis').addClass(
							'vis');
					if ($mck_preview_msg_content.html() === "") {
						$mck_preview_msg_content.removeClass('vis').addClass(
								'n-vis');
					}
				} else {
					$mck_preview_file_content.html("");
					$mck_preview_file_content.removeClass('vis').addClass(
							'n-vis');
				}
		...
		function MckInitializeChannel($this) {
			...
			var $mck_message_inner = $applozic("#mck-message-cell .mck-message-inner-right");
			_this.init = function() {
				var port = (!mckUtils.startsWith(MCK_WEBSOCKET_URL, "https")) ? "15674"
						: "15675";
				if (typeof w.SockJS === 'function') {
					var socket = new SockJS(MCK_WEBSOCKET_URL + ":" + port
							+ "/stomp");
					stompClient = w.Stomp.over(socket);
					stompClient.heartbeat.outgoing = 0;
					stompClient.heartbeat.incoming = 0;
					stompClient.connect("guest", "guest", _this.onConnect,
							_this.onError, '/');
				...
				if (!stompClient.connected) {
					if ($mck_sidebox.css('display') === 'block') {
						...
						if (currTabId) {
							...
							var conversationId = $mck_message_inner
									.data('mck-conversationid');
							var topicId = $mck_message_inner
									.data('mck-topicid');
							mckStorage.clearMckMessageArray();
							mckMessageLayout.loadTab({
								'tabId' : currTabId,
			...
			_this.subscibeToTypingChannel = function(tabId, isGroup) {
				...
				if (stompClient && stompClient.connected) {
					typingSubscriber = stompClient.subscribe("/topic/typing-"
							+ MCK_APP_ID + "-" + subscribeId,
							_this.onTypingStatus);
				} else {
					_this.reconnect();
				}
			};
			_this.sendTypingStatus = function(status, tabId) {
				if (stompClient && stompClient.connected) {
					if (status === 1 && MCK_TYPING_STATUS === 1) {
						stompClient.send('/topic/typing-' + MCK_APP_ID + "-"
								+ TYPING_TAB_ID, {
							"content-type" : "text/plain"
						}, MCK_APP_ID + "," + MCK_USER_ID + "," + status);
					}
					if (tabId) {
						if (tabId === TYPING_TAB_ID
								&& status === MCK_TYPING_STATUS && status === 1) {
							return;
						}
						TYPING_TAB_ID = tabId;
						stompClient.send('/topic/typing-' + MCK_APP_ID + "-"
			...
			_this.onTypingStatus = function(resp) {
				...
				var currTabId = $mck_message_inner.data('mck-id');
				var isGroup = $mck_message_inner.data('isgroup');
				if (!MCK_BLOCKED_TO_MAP[publisher]
						&& !MCK_BLOCKED_BY_MAP[publisher]) {
					if (status === 1) {
						if ((MCK_USER_ID !== publisher || !isGroup)
								&& (currTabId === publisher || currTabId === tabId)) {
							var isGroup = $mck_message_inner.data('isgroup');
							$mck_tab_title.addClass("mck-tab-title-w-typing");
							$mck_tab_status.removeClass('vis')
									.addClass('n-vis');
							if (isGroup) {
								if (publisher !== MCK_USER_ID) {
									var displayName = mckMessageLayout
											.getTabDisplayName(publisher, false);
									displayName = displayName.split(" ")[0];
									$mck_typing_box_text.html(displayName
											+ " is ");
								}
							} else {
								$mck_typing_box_text.html("");
							}
							$mck_typing_box.removeClass('n-vis')
									.addClass('vis');
							setTimeout(function() {
								$mck_tab_title
										.removeClass("mck-tab-title-w-typing");
								$mck_typing_box.removeClass('vis').addClass(
										'n-vis');
								if ($mck_tab_title
										.hasClass("mck-tab-title-w-status")) {
									$mck_tab_status.removeClass('n-vis')
											.addClass('vis');
								}
								$mck_typing_box_text.html("");
							}, 60000);
			...
			_this.onConnect = function() {
				if (stompClient.connected) {
					...
					subscriber = stompClient.subscribe("/topic/" + MCK_TOKEN,
							_this.onMessage);
					_this.sendStatus(1);
					_this.checkConnected();
				} else {
					setTimeout(function() {
						subscriber = stompClient.subscribe("/topic/"
								+ MCK_TOKEN, _this.onMessage);
						_this.sendStatus(1);
						_this.checkConnected();
					}, 5000);
				}
			...
			_this.onMessage = function(obj) {
				...
				if (messageType === "APPLOZIC_04"
						|| messageType === "MESSAGE_DELIVERED") {
					$applozic(
							"." + resp.message.split(",")[0]
									+ " .mck-message-status").removeClass(
							'mck-icon-time').removeClass('mck-icon-sent')
							.addClass('mck-icon-delivered');
					mckMessageLayout.addTooltip(resp.message.split(",")[0]);
					events.onMessageDelivered({
						'messageKey' : resp.message.split(",")[0]
					});
				} else if (messageType === "APPLOZIC_08"
						|| messageType === "MT_MESSAGE_DELIVERED_READ") {
					$applozic(
							"." + resp.message.split(",")[0]
									+ " .mck-message-status").removeClass(
							'mck-icon-time').removeClass('mck-icon-sent')
							.removeClass('mck-icon-delivered').addClass(
									'mck-icon-read');
					mckMessageLayout.addTooltip(resp.message.split(",")[0]);
					...
					if (typeof userId !== 'undefined') {
						mckMessageLayout
								.removeConversationThread(userId, false);
					...
					if (!MCK_BLOCKED_TO_MAP[userId]
							&& !MCK_BLOCKED_BY_MAP[userId]) {
						if (tabId === contact.contactId
								&& !$mck_message_inner.data('isgroup')) {
							$applozic("#mck-tab-status").html("Online");
						} else {
							var htmlId = mckContactUtils
									.formatContactId(userId);
							$applozic("#li-user-" + htmlId + " .mck-ol-status")
									.removeClass('n-vis').addClass('vis');
						}
						$applozic(".mck-user-ol-status." + htmlId).removeClass(
								'n-vis').addClass('vis');
						$applozic(".mck-user-ol-status." + htmlId).next().html(
								'(Online)');
					...
					var userId = resp.message;
					var contact = mckMessageLayout.fetchContact(userId);
					mckMessageLayout.updateUnreadCount('user_'
							+ contact.contactId, 0, true);
					var tabId = $mck_message_inner.data('mck-id');
					if ((typeof tabId === "undefined") || tabId === "") {
						$applozic(
								"#li-user-" + contact.htmlId
										+ " .mck-unread-count-text").html(
								mckMessageLayout.getUnreadCount('user_'
										+ contact.contactId));
						$applozic(
								"#li-user-" + contact.htmlId
										+ " .mck-unread-count-box")
					...
					if (tabId === userId) {
						$applozic(".mck-msg-right .mck-message-status")
								.removeClass('mck-icon-time').removeClass(
										'mck-icon-sent').removeClass(
										'mck-icon-delivered').addClass(
										'mck-icon-read');
						$applozic(".mck-msg-right .mck-icon-delivered").attr(
					...
					if (tabId === contact.contactId) {
						if (status === BLOCK_STATUS_MAP[0]) {
							...
							$mck_tab_status.removeClass('vis')
									.addClass('n-vis');
							$mck_typing_box.removeClass('vis')
									.addClass('n-vis');
						}
					} else {
						$applozic(
								"#li-user-" + contact.htmlId
										+ " .mck-ol-status").removeClass('vis')
								.addClass('n-vis');
					}
					...
					if (tabId === contact.contactId) {
						if (status === BLOCK_STATUS_MAP[2]) {
							...
							if (!MCK_BLOCKED_TO_MAP[tabId]) {
								...
								$mck_tab_title
										.addClass("mck-tab-title-w-status");
								$mck_tab_status.removeClass('n-vis').addClass(
										'vis');
							}
						}
					} else if (w.MCK_OL_MAP[tabId]) {
					...
					if (messageType === "APPLOZIC_03") {
						if (message.type !== 0 && message.type !== 4) {
							$applozic(
									...
									.removeClass('mck-icon-time').addClass(
											'mck-icon-sent');
					...
					} else if (messageType === "APPLOZIC_01"
						...
						if (!$mck_sidebox_content.hasClass('n-vis')) {
							if (typeof contact === 'undefined') {
								...
								if (message.groupId) {
									mckGroupLayout
											.getGroupFeedFromMessage(params);
								} else {
									var userIdArray = [];
								...
								if (message.conversationId) {
									var conversationPxy = MCK_CONVERSATION_MAP[message.conversationId];
									if ((IS_MCK_TOPIC_HEADER || IS_MCK_TOPIC_BOX)
											&& ((typeof conversationPxy !== 'object') || (typeof (MCK_TOPIC_DETAIL_MAP[conversationPxy.topicId]) !== 'object'))) {
										mckMessageService
												.getTopicId({
													'conversationId' : message.conversationId,
													'messageType' : messageType,
								...
								mckMessageLayout.populateMessage(messageType,
										message, resp.notifyUser);
							}
						}
					}
				}
		...
		function MckDateUtils() {
			...
			_this.getDate = function(createdAtTime) {
				...
				return ((currentDate.getDate() === date.getDate())
						&& (currentDate.getMonth() === date.getMonth()) && (currentDate
						.getYear() === date.getYear())) ? dateFormat(date,
						onlyTimeFormat, false) : dateFormat(date,
			...
			_this.getLastSeenAtStatus = function(lastSeenAtTime) {
				...
				if ((currentDate.getDate() === date.getDate())
					...
					var timeDiff = w.Math.floor((currentDate.getTime() - date
							.getTime()) / 60000);
					if (timeDiff < 60) {
						return (timeDiff <= 1) ? "Last seen 1 min ago"
								: "Last seen " + timeDiff + " mins ago";
					}
					return (hoursDiff === 1) ? "Last seen 1 hour ago"
							: "Last seen " + hoursDiff + " hours ago";
					...
					return "Last seen on "
							+ dateFormat(date, onlyDateFormat, false);
				}
			};
			...
			var dateFormat = function() {
				...
				return function(date, mask, utc) {
					...
					var _ = utc ? "getUTC" : "get", d = date[_ + "Date"](), D = date[_
						...
						o : (o > 0 ? "-" : "+")
								+ pad(w.Math.floor(w.Math.abs(o) / 60) * 100
										+ w.Math.abs(o) % 60, 4),
